...
try {
    ...
    // Fetch all tickets merged into this ticket
    $stmtMerged = $pdo->prepare("
        SELECT t.id, t.ticket_number, t.title, t.created_at 
        FROM ticket_relations tr
        JOIN tickets t ON tr.related_ticket_id = t.id
        WHERE tr.ticket_id = ? AND tr.relation_type = 'merged'
    ");
    $stmtMerged->execute([$ticket_id]);
    $merged_tickets = $stmtMerged->fetchAll();

    // Check if this ticket is merged into another parent ticket
    $stmtParent = $pdo->prepare("
        SELECT t.id, t.ticket_number, t.title 
        FROM ticket_relations tr
        JOIN tickets t ON tr.ticket_id = t.id
        WHERE tr.related_ticket_id = ? AND tr.relation_type = 'merged'
        LIMIT 1
    ");
    $stmtParent->execute([$ticket_id]);
    $parent_ticket = $stmtParent->fetch();
...
// Handle unmerging/separating a ticket
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unmerge_ticket'])) {
    $sub_ticket_id = intval($_POST['unmerge_ticket_id'] ?? 0);
    if ($sub_ticket_id > 0 && $user_role !== 'Müşteri Kullanıcısı') {
        try {
            // Remove relationship
            $stmtDelRel = $pdo->prepare("DELETE FROM ticket_relations WHERE ticket_id = ? AND related_ticket_id = ? AND relation_type = 'merged'");
            $stmtDelRel->execute([$ticket_id, $sub_ticket_id]);

            // Restore status of sub bilet to 'Atama bekliyor' (ID 2)
            $stmtRestoreStatus = $pdo->prepare("UPDATE tickets SET status_id = 2 WHERE id = ?");
            $stmtRestoreStatus->execute([$sub_ticket_id]);

            // Fetch ticket numbers for comments
            $stmtNums = $pdo->prepare("SELECT id, ticket_number FROM tickets WHERE id IN (?, ?)");
            $stmtNums->execute([$ticket_id, $sub_ticket_id]);
            $tickets_info = $stmtNums->fetchAll(PDO::FETCH_KEY_PAIR);

            $parent_num = $tickets_info[$ticket_id] ?? '';
            $child_num = $tickets_info[$sub_ticket_id] ?? '';

            // Add private comment to both
            $msg1 = "[AYIRMA] #{$child_num} numaralı talep bu talepten ayrılmıştır.";
            $stmtMsg1 = $pdo->prepare("INSERT INTO ticket_messages (ticket_id, sender_id, message_type, content) VALUES (?, ?, 'private', ?)");
            $stmtMsg1->execute([$ticket_id, $user_id, $msg1]);

            $msg2 = "[AYIRMA] Bu talep #{$parent_num} talebinden ayrılmıştır ve yeniden etkinleştirilmiştir.";
            $stmtMsg2 = $pdo->prepare("INSERT INTO ticket_messages (ticket_id, sender_id, message_type, content) VALUES (?, ?, 'private', ?)");
            $stmtMsg2->execute([$sub_ticket_id, $user_id, $msg2]);

            logActivity($pdo, "Bilet #{$child_num}, #{$parent_num} biletinden ayrıldı.", "tickets", $sub_ticket_id);
            
            header("Location: /destek_as/admin/ticket_detay.php?id=" . $ticket_id);
            exit;
        } catch (\Exception $e) {
            $error = "Ayırma işlemi başarısız: " . $e->getMessage();
        }
    }
}

// Handle customer canned action (approve/revision) from "Müşteriden bilgi bekleniyor" status
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['customer_canned_action']) && $user_role === 'Müşteri Kullanıcısı') {
    $cust_action = $_POST['customer_canned_action'];
    if ($ticket['status_id'] == 5) {
        try {
            if ($cust_action === 'approve') {
                $stmtUp = $pdo->prepare("UPDATE tickets SET status_id = 10 WHERE id = ?");
                $stmtUp->execute([$ticket_id]);
                
                $content_msg = "[Sistem] Müşteri çözümü onayladı ve talebi çözüldü olarak işaretledi.";
                $stmtMsg = $pdo->prepare("INSERT INTO ticket_messages (ticket_id, sender_id, message_type, content) VALUES (?, ?, 'public', ?)");
                $stmtMsg->execute([$ticket_id, $user_id, $content_msg]);

                logActivity($pdo, "Bilet müşteri tarafından onaylanıp kapatıldı.", "tickets", $ticket_id);
            } elseif ($cust_action === 'revision') {
                $stmtUp = $pdo->prepare("UPDATE tickets SET status_id = 2 WHERE id = ?");
                $stmtUp->execute([$ticket_id]);

                $content_msg = "[Sistem] Müşteri revize istedi; bilet yeniden 'Atama bekliyor' aşamasına alınarak işleme sokuldu.";
                $stmtMsg = $pdo->prepare("INSERT INTO ticket_messages (ticket_id, sender_id, message_type, content) VALUES (?, ?, 'public', ?)");
                $stmtMsg->execute([$ticket_id, $user_id, $content_msg]);

                logActivity($pdo, "Bilet müşteri tarafından revize istendi, yeniden 'Atama bekliyor' yapıldı.", "tickets", $ticket_id);
            }
...
<div style="display: flex; flex-direction: column; gap: 30px;">
    ...
    <?php if ($user_role === 'Müşteri Kullanıcısı' && $ticket['status_id'] == 5): ?>
        <div style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.15) 0%, rgba(217, 119, 6, 0.05) 100%); border: 1px solid var(--warning); padding: 20px; border-radius: 12px; display: flex; align-items: center; justify-content: space-between; gap: 20px; box-shadow: 0 4px 20px rgba(245,158,11,0.15);">
            <div>
                <h4 style="color: var(--primary); font-size: 15px; margin-bottom: 6px; font-weight: 700;"><i class="fa-solid fa-circle-info"></i> Bilgi/Onay Talebi Bulunuyor</h4>
                <p style="color: var(--text-secondary); font-size: 13px; line-height: 1.5; margin: 0;">Bu bilet için uzman ekibimiz sizden **bilgi veya onay** beklemektedir. Sorununuz çözüldüyse **Onayla** seçeneği ile kapatabilir, ya da **Revize İste** seçeneği ile talebinizi en başa (Atama bekliyor durumuna) çekip yeniden işleme aldırabilirsiniz.</p>
            </div>
            <div style="display: flex; gap: 10px; flex-shrink: 0;">
                <form method="POST" action="" style="margin: 0;">
                    <input type="hidden" name="customer_canned_action" value="approve">
                ...
                <form method="POST" action="" style="margin: 0;" onsubmit="return confirm('Bu talebe revize isteyerek yeniden işleme aldırmak istediğinize emin misiniz?');">
                    <input type="hidden" name="customer_canned_action" value="revision">
                    <button type="submit" class="btn-custom" style="background-color: var(--danger); color: #fff; border: none; padding: 10px 18px; border-radius: 8px; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; font-size: 12px; transition: transform 0.2s;">
                        <i class="fa-solid fa-arrows-rotate"></i> Revize İste
                    </button>
                </form>
            </div>
        </div>
    ...
    <div style="display: grid; grid-template-columns: 2.2fr 1fr; gap: 30px; align-items: flex-start;">
        ...
        <div style="display: flex; flex-direction: column; gap: 30px;">
            ...
            <div class="glass-card">
                ...
                <div style="font-size: 15px; line-height: 1.7; background: rgba(178, 94, 44, 0.04); padding: 20px; border-radius: 12px; border: 1px solid rgba(178, 94, 44, 0.15); color: var(--text-primary); white-space: pre-wrap;"><?php echo htmlspecialchars($ticket['description']); ?></div>
                ...
                <!-- Merged Tickets Info Section -->
                <?php if (!empty($merged_tickets)): ?>
                    <div style="margin-top: 20px; padding: 15px 20px; background: rgba(178, 94, 44, 0.05); border: 1px dashed var(--primary); border-radius: 12px;">
                        <h4 style="font-size: 14px; font-weight: 700; color: var(--primary); margin: 0 0 10px 0; display: flex; align-items: center; gap: 8px;">
                            <i class="fa-solid fa-link"></i> Bu Taleple Birleştirilen Diğer Talepler (<?php echo count($merged_tickets); ?>)
                        </h4>
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            <?php foreach ($merged_tickets as $m_t): ?>
                                <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.3); padding: 10px 15px; border-radius: 8px; border: 1px solid var(--border-color);">
                                    <div>
                                        <span class="badge badge-secondary" style="font-size: 9px; padding: 2px 6px; background: var(--bg-sidebar); border: 1px solid var(--border-color); color: var(--text-secondary);">#<?php echo htmlspecialchars($m_t['ticket_number']); ?></span>
                                        <strong style="font-size: 12px; margin-left: 8px; color: var(--text-primary);"><?php echo htmlspecialchars($m_t['title']); ?></strong>
                                    </div>
                                    <div style="display: flex; gap: 8px;">
                                        <form method="POST" action="" style="margin: 0;" onsubmit="return confirm('Bu talebi ayırmak istediğinize emin misiniz?');">
                                            <input type="hidden" name="unmerge_ticket" value="1">
                                            <input type="hidden" name="unmerge_ticket_id" value="<?php echo $m_t['id']; ?>">
                                            <button type="submit" class="btn-custom btn-custom-danger" style="padding: 4px 10px; font-size: 11px; cursor: pointer; border: none;">
                                                <i class="fa-solid fa-link-slash"></i> Ayır
                                            </button>
                                        </form>
                                        <a href="/destek_as/admin/ticket_detay.php?id=<?php echo $m_t['id']; ?>" class="btn-custom btn-custom-secondary" style="padding: 4px 10px; font-size: 11px; text-decoration: none;">
                                            Detayı İncele
                                        </a>
                                    </div>
                                </div>
                ...
                <?php if (!empty($parent_ticket)): ?>
                    <div style="margin-top: 20px; padding: 15px 20px; background: rgba(239, 68, 68, 0.03); border: 1px solid var(--danger); border-radius: 12px;">
                        <h4 style="font-size: 14px; font-weight: 700; color: var(--danger); margin: 0 0 8px 0; display: flex; align-items: center; gap: 8px;">
                            <i class="fa-solid fa-code-merge"></i> Bu Talep Birleştirilmiştir
                        </h4>
                        <p style="font-size: 13px; color: var(--text-secondary); margin: 0;">
                            Bu talep kapatılarak <strong>#<?php echo htmlspecialchars($parent_ticket['ticket_number']); ?> - <?php echo htmlspecialchars($parent_ticket['title']); ?></strong> talebiyle birleştirilmiştir.
                            <a href="/destek_as/admin/ticket_detay.php?id=<?php echo $parent_ticket['id']; ?>" style="color: var(--primary); font-weight: 700; text-decoration: none; margin-left: 5px;">
                                Ana Talebe Git <i class="fa-solid fa-circle-arrow-right"></i>
                            </a>
                        </p>
                    </div>
                <?php endif; ?>
        ...
        <div style="display: flex; flex-direction: column; gap: 30px;">
            <?php if ($user_role !== 'Müşteri Kullanıcısı'): ?>
                ...
                <div class="glass-card" style="display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <div style="background: rgba(178, 94, 44, 0.04); padding: 15px; border-radius: 12px; border: 1px solid rgba(178, 94, 44, 0.15); display: flex; flex-direction: column; gap: 8px; font-size: 13px;">
                        ...
                        <div style="border-top: 1px solid rgba(178, 94, 44, 0.15); margin-top: 8px; padding-top: 8px;">
                            Durum: 
